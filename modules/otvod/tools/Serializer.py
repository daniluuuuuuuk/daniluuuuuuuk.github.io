"""Классы используются для сохранения/загрузки данных отвода и точек лесосеки
Данные отвода хранятся в таблице public.area_data, 
точки - в public.area_points
Загрузка и сохранение данных нужно для экспорта лесосек и печати картосхемы инстурментом 
        ...
        def workerFinished(result):
            ...
            if not result:
                QMessageBox.information(
                    None, "Ошибка", "Данные отвода для лесосеки не найдены."
                )
                return
...
class Worker(QtCore.QObject):
    def __init__(self, taskType):
        ...
        self.data = None
        self.serializer = SerializerTask(
            "Work with serializable area", self.taskType
        )
        ...
        try:
            ...
            if self.serializer.area == None and self.taskType != "Save":
                QgsMessageLog.logMessage(
                    '\nError "{}"'.format(
                        "Ошибка, данные точки привязки не найдены"
                    ),
                    MESSAGE_CATEGORY,
                    Qgis.Warning,
                )
                ...
                QgsMessageLog.logMessage(
                    '\nError "{}"'.format("Ошибка, данные отвода не найдены"),
                    MESSAGE_CATEGORY,
                    Qgis.Warning,
                )
                ret = []
...
class SerializerTask(QgsTask):
    ...
    def saveAreaToDatabase(self, data):
        try:
            ...
            cursor.execute(
                "INSERT INTO {table} "
                "VALUES ('{uuid}', {type}, {coord}, {x}, {y}, {inclination})".format(
                    table=AREA_DATA_TABLE_NAME,
                    uuid=data[0],
                    type=data[1],
                    coord=data[2],
                    x=data[3],
            ...
            for key, value in data[-1].items():
                cursor.execute(
                    "INSERT INTO {table} "
                    "VALUES ('{area_uuid}', {pt_num}, {x}, {y}, '{point_type}')".format(
                        table=AREA_POINTS_TABLE_NAME,
                        area_uuid=data[0],
                        pt_num=key,
                        x=value[0].x(),
                        y=value[0].y(),
    ...
    def loadAreaFromDatabase(self, uid):
        try:
            ...
            areaData = postgisConnection.getQueryResult(
                """SELECT * FROM {table} where area_uid = '{uid}'""".format(
                    table=AREA_DATA_TABLE_NAME, uid=uid
                )
            )
            if not areaData:
                self.area = None
            ...
            points = postgisConnection.getQueryResult(
                """SELECT * FROM {table} where area_uid = '{uid}'""".format(
                    table=AREA_POINTS_TABLE_NAME, uid=uid
                )
            )
    ...
    def prepareData(self, data, points):
        ...
        for point in points:
            pointsDict.update(
                {
                    point[1]: [
                        QgsPointXY(float(point[2]), float(point[3])),
                        point[4],
                    ]
                }
            )
