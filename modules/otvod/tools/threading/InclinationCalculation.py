...
class Calculator(QgsTask):
    ...
    def calculateAsAzimuths(self, tableList, coordType, inclinationValue):
        for row in tableList:
            ...
            if coordType == 1:
                azDec = (
                    GeoOperations.convertDMSAngle(row[1], row[2], row[3])
                    - inclinationValue
                )
    ...
    def calculateAsRumbs(self, tableList, coordType, inclinationValue):
        for row in tableList:
            if coordType == 0:
                az = (
                    GeoOperations.rumbToAzimuth(row[3], row[1])
                    - inclinationValue
                )
            ...
            if coordType == 1:
                ...
                decAz = (
                    GeoOperations.rumbToAzimuth(row[5], decRumb)
                    - inclinationValue
                )
    ...
    def getFirstPointOfArea(self, tableList):
        ...
        for i, row in enumerate(tableList):
            if i == 0 and row[-1] == "Лесосека":
                ...
                i > 0
                and tableList[i - 1][-1] == "Привязка"
                and tableList[i][-1] == "Лесосека"
            ):
                firstPointOfArea = i
