...
class Settings_caliper(
    QtWidgets.QDialog,
    uic.loadUiType(BasicDir().get_basic_dir("gui/settings_caliper.ui"))[0],
):
    def __init__(self, parent=None):
        ...
        self.comboBox.addItems(
            Settings(group="Caliper", key="existing_ports").read_setting()
        )
        ...
        self.pushButton_2.clicked.connect(
            lambda x: [
                Settings(group="Default_Settings", key="caliper_port").add_setting(
        ...
        try:
            ...
            if ser.getCTS() or self.ser.getDSR():
                QtWidgets.QMessageBox.information(
                    None, "Успешно", "Подключение к вилке успешно."
                )
            else:
                QtWidgets.QMessageBox.critical(
                    None, "Ошибка", "Ошибка подключения к вилке"
                )
        except:
