...
class AddByHand(
    QtWidgets.QDialog, uic.loadUiType(BasicDir().get_basic_dir("gui/add_by_hand.ui"))[0]
):
    def __init__(self, last_data, parent=None):
        ...
        try:
            if (
                self.last_data["num_ind"] != 0
                or self.last_data["num_half_ind"] != 0
                or self.last_data["num_fuel"] != 0
            ):
