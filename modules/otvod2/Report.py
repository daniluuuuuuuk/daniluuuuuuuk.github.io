...
class Report:
    ...
    def __init__(self, table, canvas, layers):
        ...
        self.columnNames = self.table.getColumnNames()
        self.layers = layers
    ...
    def generate(self):
        ...
        p = document.add_paragraph(
            ...
            ', лесной квартал №{}, таксационный выдел №{}, площадь лесосеки {} га.'.format(str(self.areaAttributes["num_kv"]), str(self.areaAttributes["num_vd"]), str(self.areaAttributes["area"]))
            )
        ...
        self.add_border(in_img, output_image=os.path.join(QgsProject.instance().homePath(), "outputborder.png"), border=1)

        document.add_picture(os.path.join(QgsProject.instance().homePath(), "outputborder.png"))
        last_paragraph = document.paragraphs[-1]
        last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER

        p = document.add_paragraph('Условные обозначения:')
        
        p = document.add_paragraph('Экспликация или координаты поворотных точек лесосеки:')
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        path = os.path.join(QgsProject.instance().homePath(), 'Отвод_кв_{}.docx'.format(str(self.areaAttributes["num_kv"])))
        document.save(path)
        path = path.replace("\\", "\\\\")
        path = path.replace("/", "\\\\")
    ...
    def getFeatureByUid(self):
        try:
            ...
            QMessageBox.information(None, "Ошибка модуля QGis", "Лесосека с таким идентификатором не найдена " + str(self.uid))
    ...
    def getAreaAttributes(self):
        try:
            ...
            QMessageBox.information(None, "Ошибка модуля QGis", "Сначала сохраните лесосеку " + str(e))
