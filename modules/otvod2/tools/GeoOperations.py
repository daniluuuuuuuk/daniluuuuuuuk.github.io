...
def convertToZone35(point):
  ...
  transf = QgsCoordinateTransform(QgsCoordinateReferenceSystem(4326), QgsCoordinateReferenceSystem(32635), QgsProject.instance())
  wgsPoint = transf.transform(point)
  return wgsPoint

def convertToWgs(point):
  transf = QgsCoordinateTransform(QgsCoordinateReferenceSystem(32635), QgsCoordinateReferenceSystem(4326), QgsProject.instance())
  wgsPoint = transf.transform(point)
  return wgsPoint
...
def getCoordFromGPS():
  try:
      ...
      QMessageBox.information(None, 'Ошибка подключения', "Отсутствует подключение к GPS. Нажмите CTRL + 0 для настройки")
