"""Классы используются для поиска лесосек и навигации по результатам 
(вкладка "фильтр лесосек" панели инструментов модуля)
Для поиска используется конструктор запросов QGIS, который обращается 
к слою лесосек. В качестве критериев для поиска используются основные поля слоя лесосек
По найденным лесосекам можно осуществлять навигацию (пред./след) и экспорт 
"""
