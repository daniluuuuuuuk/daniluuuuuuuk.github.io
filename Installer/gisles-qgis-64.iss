...
[Messages]
WelcomeLabel2=Программное обеспечение для отвода и таксации участков лесного фонда ГисЛес для QGIS предназначается для автоматизации процессов определения границ участков лесного фонда и перечета деревьев. Выполняет функции сбора, ввода, преобразования и хранения данных.
...
[Tasks]
Name: qgis; Description: "Установка QGIS Desktop"
Name: module; Description: "Установка модуля ГисЛес для QGIS"
Name: postgresql; Description: "Установка PostgreSQL локально"
Name: dbeaver; Description: "Установка клиента БД DBeaver CE"; Flags: unchecked

[Run]   
Filename: "{tmp}\{#vc_redist_file}"; StatusMsg: "Установка VC Redist 2015"; \
  Parameters: "/q /norestart"; Check: VC2015RedistNeedsInstall ; Flags: waituntilterminated
 
Filename: "{tmp}\{#QGIS_setup_file}"; StatusMsg: "Устанока QGIS Desktop"; \
  Parameters: "/S"; Flags: waituntilterminated;  Tasks: qgis

Filename: "{tmp}\{#postgresql_file}"; StatusMsg: "Установка PostgreSQL"; \
  Parameters: " --mode unattended --unattendedmodeui none --superpassword {#PsqlPass}"; Flags: waituntilterminated; Tasks: postgresql

Filename: "{tmp}\{#postgis_file}"; StatusMsg: "Установка PostGis"; \
  Parameters: "/S"; Flags: waituntilterminated; Tasks: postgresql
  
Filename: "{tmp}\unzip.exe"; StatusMsg: "Установка модуля ГисЛес для QGIS"; \
...
Filename: "{tmp}\{#dbeaver_file}"; StatusMsg: "Установка DBeaver"; \
